...
class Auth_admin_model extends CI_Model {
    ...
    public function add_item_list(){
        ...
        $this->db->update('items_list',array('board_ids'=>$this->clearString($this->input->post('boardId'))),array('created_by'=>$login_data['id'], 'status'=>0));
    ...
    function save_user(){
        ...
        $data = array(
                ...
                'user_name'=>trim(strtolower($this->input->post('user_name'))),
                'password'=>$password,
                'board_id'=>$boardIds,
    ...
    public function get_board_message(){
        ...
        $this->db->select('board_id');
        $this->db->from('users');
        $condition = array('id' => $login_data['id']);
        $this->db->where($condition);
        $data = $this->db->get()->row_array();
        $dataarr = explode(',',$data['board_id']);
        $board_message = array();
    ...
    public function add_message_master(){
        ...
        $this->db->select('id');
        $this->db->from('board_message');
        $this->db->where(array('board_id'=>$this->input->post('board'), 'language'=>$this->input->post('language')));
        $boarddata = $this->db->get()->row();
